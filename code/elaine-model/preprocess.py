...
class Datasets():
    ...
    def calc_mean_and_std(self):
        ...
        # calculate the pixel-wise mean and standard deviation of the images in data_sample
        # store them in self.mean and self.std
    ...
    def get_data(self, path, is_vgg, shuffle, augment):
        ...
        if augment:
            # use the arguments of ImageDataGenerator() to augment the data
            data_gen = tf.keras.preprocessing.image.ImageDataGenerator(
                rotation_range=8, zoom_range=0.05, horizontal_flip=True,
                preprocessing_function=self.preprocess_fn)
            
            # regularize data between -1 and 1?
            # ============================================================
        else:
            # Don't modify this
            data_gen = tf.keras.preprocessing.image.ImageDataGenerator(
